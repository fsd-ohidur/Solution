...
namespace Solution.Web.CNF.Controllers
{
	public class DepotController : Controller
	{
		...
		public async Task<IActionResult> Index()
		{
			...
			var response = await _Service.Depots.GetAllAsync(ComId, UserId, route);
		...
		public async Task<IActionResult> Details(string id)
		{
			var ComId = Request.Cookies["ComId"];
			var UserId = Request.Cookies["UserId"];
			var model = await _Service.Depots.GetByIdAsync(id, ComId, UserId, route);
			if (model == null)
			{
				return NotFound();
			}
		...
		public async Task<IActionResult> Create(DepotDto model)
		{
			if (ModelState.IsValid)
			{
				...
				model.ComId = ComId;
				var response = await _Service.Depots.CreateAsync(model, ComId, UserId, route);
				if (response != null && response.IsSuccess)
				{
		...
		public async Task<IActionResult> Edit(string id)
		{
			...
			var response = await _Service.Depots.GetByIdAsync(id, ComId, UserId, route);
			if (response != null && response.IsSuccess)
			{
		...
		public async Task<IActionResult> Edit(DepotDto model)
		{
			if (ModelState.IsValid)
			{
				...
				model.ComId = ComId;
				var response = await _Service.Depots.UpdateAsync(model, ComId, UserId, route);
				if (response != null && response.IsSuccess)
				{
		...
		public async Task<IActionResult> Delete(DepotDto model)
		{
			...
			var response = await _Service.Depots.DeleteAsync(model.Id, ComId, UserId, route);
