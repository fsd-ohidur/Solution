...
namespace Solution.Web.HR.Controllers
{
	public class DepartmentController : Controller
	{
		...
		public async Task<IActionResult> Index()
		{
			...
			var response = await _Service.Departments.GetAllAsync(SD.HRAPIBase, ComId,UserId, route);
		...
		public async Task<IActionResult> Details(string id)
		{
			var ComId = Request.Cookies["ComId"];
			var UserId = Request.Cookies["UserId"];
			var model = await _Service.Departments.GetByIdAsync(SD.HRAPIBase, id,ComId,UserId, route);
			if (model == null)
			{
				return NotFound();
			}
		...
		public async Task<IActionResult> Create(DepartmentDto model)
		{
			if (ModelState.IsValid)
			{
				...
				model.ComId = ComId;
				var response = await _Service.Departments.CreateAsync(SD.HRAPIBase, model,ComId,UserId, route);
				if (response != null && response.IsSuccess)
				{
		...
		public async Task<IActionResult> Edit(string id)
		{
			...
			var response = await _Service.Departments.GetByIdAsync(SD.HRAPIBase, id, ComId, UserId, route);
			if (response != null && response.IsSuccess)
			{
		...
		public async Task<IActionResult> Edit(DepartmentDto model)
		{
			if (ModelState.IsValid)
			{
				...
				model.ComId = ComId;
				var response = await _Service.Departments.UpdateAsync(SD.HRAPIBase, model, ComId, UserId, route);
				if (response != null && response.IsSuccess)
				{
		...
		public async Task<IActionResult> Delete(DepartmentDto model)
		{
			...
			var response = await _Service.Departments.DeleteAsync(SD.HRAPIBase, model.Id, ComId, UserId, route);
